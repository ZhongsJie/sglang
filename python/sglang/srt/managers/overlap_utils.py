...
from python.sglang.srt.speculative.suffix_info_v2 import SuffixDraftInputV2
import torch
...
class FutureMap:
    ...
    def _lazy_init_buf_for_suffix(self, draft_input: SuffixDraftInputV2):
        self.buf_initialized = True
    ...
    def resolve_future(self, model_worker_batch: ModelWorkerBatch):
        ...
        elif self.spec_algo.is_suffix():
            # TODO zhongsjie
            draft_input: SuffixDraftInputV2 = model_worker_batch.spec_info
            if draft_input is None:
                return
            indices = draft_input.future_indices.indices
            draft_input.verified_id = self.verified_id_buf[indices]
    ...
    def store_to_map(
        ...
        if self.spec_algo.is_suffix():
            # TODO zhongsjie
            draft_input: SuffixDraftInputV2 = batch_result.next_draft_input
            self.store_to_map_for_new_batch_with_suffix(future_indices, draft_input)
        else:
            intv = future_indices.interval
            self.token_ids_buf[intv] = batch_result.next_token_ids

    def store_to_map_for_new_batch_with_suffix(
        self, future_indices: FutureIndices, draft_input: SuffixDraftInputV2
    ):
        intv = future_indices.interval
        ...
        if not self.buf_initialized:
            self._lazy_init_buf_for_suffix(draft_input)
